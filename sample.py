...
def get_window_under_cursor():
    cursor_x, cursor_y = pyautogui.position()
    window = win32gui.WindowFromPoint((cursor_x, cursor_y))
    return window

# Function to get the screenshot of a specific window
...
# Get the screenshot of the window under the cursor
window_screenshot = get_window_screenshot(window)

# Get the cursor position
cursor_x, cursor_y = pyautogui.position()

# Convert the window screenshot to grayscale
gray = cv2.cvtColor(window_screenshot, cv2.COLOR_BGR2GRAY)
...
for cnt in contours:
    ...
    # Check if the cursor is inside the bounding rectangle
    if x <= cursor_x <= x + w and y <= cursor_y <= y + h:
        cv2.rectangle(window_screenshot, (x, y), (x + w, y + h), (0, 255, 0), 2)
        
        # Extract the image within the bounding rectangle
        roi = window_screenshot[y:y + h, x:x + w]
        
        # Save the extracted image as "cursor_object.png"
        cv2.imwrite(f"cursor_object.png", roi)
        i += 1

cv2.imshow('Window under Cursor', window_screenshot)
cv2.waitKey(0)
cv2.destroyAllWindows()
